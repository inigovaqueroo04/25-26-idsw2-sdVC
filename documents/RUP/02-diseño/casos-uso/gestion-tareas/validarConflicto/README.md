...
| Caso de uso relacionado | validarConflicto() |
| Módulo funcional | Gestión de tareas |

## Propósito

Diseñar la detección conceptual de solapamientos de horario para usuarios asignados.
...
| Coordinador del caso de uso | Control | Ordena el flujo de validarConflicto() y decide qué servicio invocar. |
...
| Entidades de dominio implicadas | Dominio | Tarea, Horario, ConflictoHorario, Usuario. |

## Decisiones de diseño

- Datos necesarios: tarea evaluada, horario, asignaciones y tareas existentes del usuario.
- Entidades afectadas: Tarea, Horario, ConflictoHorario, Usuario.
- Cambios de estado: TAREA_ABIERTO, PLANIFICACION_ABIERTO, CONFLICTO_DETECTADO.
...
- Decisión específica: Los conflictos se diseñan como avisos idempotentes derivados de horarios y asignaciones.
...
1. El usuario solicita validarConflicto() desde la interfaz.
...
TAREA_ABIERTO, PLANIFICACION_ABIERTO, CONFLICTO_DETECTADO

## Validaciones

- La tarea debe tener horario comparable.
- Las asignaciones deben estar definidas.
- Solo se comparan tareas relevantes del mismo usuario.

## Excepciones o errores

- Horario incompleto.
- Asignación no disponible.
- No se puede calcular comparación temporal.

## Resultado esperado

Los conflictos quedan registrados o actualizados sin bloquear por defecto la operación principal.

## Trazabilidad

| Elemento de análisis usado | Decisión de diseño derivada | Entidad o módulo afectado | Estado |
| --- | --- | --- | --- |
| [README de análisis](../../../../01-analisis/casos-uso/gestion-tareas/validarConflicto/README.md) | Convertir el comportamiento funcional en colaboración entre interfaz, coordinador y servicios conceptuales. | Gestión de tareas | diseñado |
| [Diagrama de colaboración](../../../../01-analisis/casos-uso/gestion-tareas/validarConflicto/colaboracion.puml) | Mantener separación entre límite, control y dominio sin fijar tecnología. | CoordinadorValidarConflicto, ServicioAplicacion, ServicioDominio | diseñado |
| [Secuencia de análisis](../../../../01-analisis/casos-uso/gestion-tareas/validarConflicto/secuencia.puml) | Preservar orden de interacción y estados principales. | EstadoAplicacion, Tarea, Horario, ConflictoHorario, Usuario | diseñado |
...
participant "CoordinadorValidarConflicto" as Coordinador
...
Usuario -> Interfaz : solicita validarConflicto()
Interfaz -> Coordinador : enviar datos de entrada
Coordinador -> ServicioAplicacion : ejecutar validarConflicto()
ServicioAplicacion -> ServicioDominio : validar reglas del caso
ServicioDominio -> Repositorio : consultar Tarea, Horario, ConflictoHorario, Usuario
