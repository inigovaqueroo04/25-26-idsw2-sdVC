...
| Caso de uso relacionado | abrirInvitaciones() |
| Módulo funcional | Gestión de grupos y usuarios |

## Propósito

Diseñar la consulta de invitaciones pendientes o históricas del usuario.
...
| Coordinador del caso de uso | Control | Ordena el flujo de abrirInvitaciones() y decide qué servicio invocar. |
...
| Entidades de dominio implicadas | Dominio | Invitacion, Usuario, Grupo, EstadoAplicacion. |

## Decisiones de diseño

- Datos necesarios: usuario actual, filtros de estado y listado de invitaciones.
- Entidades afectadas: Invitacion, Usuario, Grupo, EstadoAplicacion.
- Cambios de estado: SISTEMA_DISPONIBLE, INVITACIONES_ABIERTO.
...
- Decisión específica: La lista de invitaciones es una vista de consulta que deriva hacia editarInvitacion().
...
1. El usuario solicita abrirInvitaciones() desde la interfaz.
...
SISTEMA_DISPONIBLE, INVITACIONES_ABIERTO

## Validaciones

- El usuario debe estar autenticado.
- Solo se consultan invitaciones dirigidas al usuario o gestionables por su rol.

## Excepciones o errores

- Usuario sin sesión.
- No existen invitaciones.
- Fallo al consultar invitaciones.

## Resultado esperado

El usuario visualiza sus invitaciones y puede seleccionar una para gestionarla.

## Trazabilidad

| Elemento de análisis usado | Decisión de diseño derivada | Entidad o módulo afectado | Estado |
| --- | --- | --- | --- |
| [README de análisis](../../../../01-analisis/casos-uso/gestion-grupos/abrirInvitaciones/README.md) | Convertir el comportamiento funcional en colaboración entre interfaz, coordinador y servicios conceptuales. | Gestión de grupos y usuarios | diseñado |
| [Diagrama de colaboración](../../../../01-analisis/casos-uso/gestion-grupos/abrirInvitaciones/colaboracion.puml) | Mantener separación entre límite, control y dominio sin fijar tecnología. | CoordinadorAbrirInvitaciones, ServicioAplicacion, ServicioDominio | diseñado |
| [Secuencia de análisis](../../../../01-analisis/casos-uso/gestion-grupos/abrirInvitaciones/secuencia.puml) | Preservar orden de interacción y estados principales. | EstadoAplicacion, Invitacion, Usuario, Grupo, EstadoAplicacion | diseñado |
...
participant "CoordinadorAbrirInvitaciones" as Coordinador
...
Usuario -> Interfaz : solicita abrirInvitaciones()
Interfaz -> Coordinador : enviar datos de entrada
Coordinador -> ServicioAplicacion : ejecutar abrirInvitaciones()
ServicioAplicacion -> ServicioDominio : validar reglas del caso
ServicioDominio -> Repositorio : consultar Invitacion, Usuario, Grupo, EstadoAplicacion
