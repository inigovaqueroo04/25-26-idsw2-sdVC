...
| Caso de uso relacionado | cerrarSesion() |
| Módulo funcional | Gestión de sesión y navegación |

## Propósito

Diseñar el cierre ordenado de una sesión activa y la vuelta a un estado no autenticado.
...
| Coordinador del caso de uso | Control | Ordena el flujo de cerrarSesion() y decide qué servicio invocar. |
...
- Datos necesarios: sesión activa e identificador del usuario actual.
- Entidades afectadas: Usuario, EstadoAplicacion, Sesion.
- Cambios de estado: SISTEMA_DISPONIBLE, SESION_CERRADA.
...
- Decisión específica: El cierre de sesión invalida el estado activo y no modifica datos de dominio.
...
1. El usuario solicita cerrarSesion() desde la interfaz.
...
SISTEMA_DISPONIBLE, SESION_CERRADA

## Validaciones

- Debe existir una sesión activa.
- La sesión debe corresponder al usuario actual.

## Excepciones o errores

- No existe sesión activa.
- Fallo al limpiar el estado de sesión.

## Resultado esperado

La sesión queda cerrada y no se conserva acceso operativo al sistema.

## Trazabilidad

| Elemento de análisis usado | Decisión de diseño derivada | Entidad o módulo afectado | Estado |
| --- | --- | --- | --- |
| [README de análisis](../../../../01-analisis/casos-uso/gestion-sesion/cerrarSesion/README.md) | Convertir el comportamiento funcional en colaboración entre interfaz, coordinador y servicios conceptuales. | Gestión de sesión y navegación | diseñado |
| [Diagrama de colaboración](../../../../01-analisis/casos-uso/gestion-sesion/cerrarSesion/colaboracion.puml) | Mantener separación entre límite, control y dominio sin fijar tecnología. | CoordinadorCerrarSesion, ServicioAplicacion, ServicioDominio | diseñado |
| [Secuencia de análisis](../../../../01-analisis/casos-uso/gestion-sesion/cerrarSesion/secuencia.puml) | Preservar orden de interacción y estados principales. | EstadoAplicacion, Usuario, EstadoAplicacion, Sesion | diseñado |
...
participant "CoordinadorCerrarSesion" as Coordinador
...
Usuario -> Interfaz : solicita cerrarSesion()
Interfaz -> Coordinador : enviar datos de entrada
Coordinador -> ServicioAplicacion : ejecutar cerrarSesion()
